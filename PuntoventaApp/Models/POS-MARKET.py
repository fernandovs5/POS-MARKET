    ...
    def actualizar_stock(self, nombre_producto, cantidad):
        if nombre_producto in self.stock:
            ...
            if self.stock[nombre_producto]['cantidad'] <= 0:
                del self.stock[nombre_producto]
            self.guardar_stock()
        else:
...
class Carrito:
    ...
    def contenido(self):
        return {nombre: {'cantidad': info['cantidad'], 'precio': info['producto'].precio} for nombre, info in self.productos.items()}
    ...
    def procesar_compra(cliente: Cliente, supermercado: Supermercado):
        ...
        if total > 0:
            for nombre, info in cliente.carrito.contenido().items():
                supermercado.actualizar_stock(nombre, info['cantidad'])
            return total
        else:
            return 0
...
class App:
    ...
    def ver_contenido_carrito(self):
        ...
        if contenido:
            contenido_info = "\n".join([f"{nombre}: {info['cantidad']} unidades a {info['precio']} cada una" for nombre, info in contenido.items()])
    ...
    def realizar_compra(self):
        ...
        if total > 0:
            self.generar_ticket()  # Generar el ticket después de la compra
    ...
    def generar_ticket(self):
        ticket_info = "----- TICKET DE VENTA -----\n"
        ticket_info += f"Cliente: {self.cliente.nombre}\n"
        ticket_info += f"Teléfono: {self.cliente.telefono}\n"
        ticket_info += "Productos:\n"
        ...
        for nombre, info in contenido.items():
            cantidad = info['cantidad']
            precio = info['precio']  # Obtener el precio del producto
            subtotal = precio * cantidad  # Calcular el subtotal
            total += subtotal  # Sumar al total
            ticket_info += f"{nombre}: {cantidad} unidades a {precio} cada una. Subtotal: {subtotal}\n"

        ticket_info += f"Total: {total}\n"
        ticket_info += "--------------------------\n"
        ...
        self.mostrar_ticket(ticket_info)

    def mostrar_ticket(self, ticket_info):
        ticket_window = Toplevel(self.root)
        ticket_window.title("Ticket de Venta")
        
        text_area = tk.Text(ticket_window, width=40, height=15)
        text_area.pack()
        text_area.insert(tk.END, ticket_info)
